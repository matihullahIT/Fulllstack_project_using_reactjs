...
const filter = (e) => {
  ...
  const filterData = data.filter(item => item.status.toLowerCase() === e.target.value.toLowerCase());
  console.log("Filtered Data:", filterData);
  setData(filterData);
};
function findTask(e){
    const filterData = data.filter(item => item.task === e.target.value);
  console.log("Filtered Data:", filterData);
  setData(filterData);
}
const EnterData=()=>{
  ...
  />
            ...
            <div className="flex flex-col items-center w-full  bg-gray-400  border-pink-300">
                ...
                auto gap-4 py-2"> 
                    ...
                    onKeyDown={(e)=>e.target.value!==""? findTask(e):fetchData()}
                    placeholder="Task"
                    />
                    <button className="px-1 py-3 border-white   rounded-xl  linear transition-all bg-blue-500 text-white hover:border-white hover:scale-110">Add A task</button>
                    <select className="bg-white px-2 py-3 text-xl rounded-md capitalize" onChange={(e)=>e.target.value? filter(e):fetchData()}>
                ...
                {data.length === 0 ? (
                    ...
                    data.map((item) => (
                        ...
                        transition-all ease-in-out hover:bg-black hover:text-white hover:scale-120 hover:z-40 not-hover:z-0 ">
                            <div>
